...
def perfect_square(score):
    """Check if the total score if a player is a perfact square number"""
    factor = 1
    while factor <= score // 2 + 1:
        if score == factor * factor:
            return True
        factor += 1
    return False    
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        score1 = 0
        while score1 < goal: 
            if num_roll != strategy(score0, score1):
                return False
            score1 += 1
        score0 += 1
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    def passargs(*args):
        sum = 0
        implement_time = 1
        while implement_time <= total_samples:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    highest_dice = 1
    highest_score = make_averaged(roll_dice, total_samples)(highest_dice, dice) 
    num_dice = 2
    while num_dice <= 10:
         averaged_score = make_averaged(roll_dice, total_samples)(num_dice, dice) 
         if  highest_score < averaged_score:
             highest_score = averaged_score
             highest_dice = num_dice
         num_dice += 1
    return highest_dice
