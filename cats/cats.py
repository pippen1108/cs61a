...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(s):
        after_s = split(lower(remove_punctuation(s)))
        for t in topic:
            if t in after_s:  
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
       ...
       autocorrect_word =  min(word_list, key=lambda x: diff_function(typed_word, x, limit))
       if diff_function(typed_word, autocorrect_word, limit) > limit:
           return typed_word
       else:
...
def report_progress(typed, prompt, user_id, upload):
    ...
    upload_dic = {'id': user_id, 'progress': progress}
    upload(upload_dic)
    return progress    
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        ...
        for index in range(len(player)-1):	
            time += [player[1:][index] - player[:-1][index]]
        times += [time] 
...
def fastest_words(match):
    ...
    for player in player_indices:
        ...
        for word in word_indices:
           if player ==  min(player_indices,key=lambda x:time(match, x, word)):
               fast_player += [get_word(match, word)] 
        fastest += [fast_player]                   
    return fastest
